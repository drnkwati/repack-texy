...
final class TypographyModule extends Texy\Module
{
    ...
    public function beforeParse(Texy\Texy $texy, &$text)
    {
        ...
        $this->pattern = array(
            ...
            '#(?<=^| |\.|,|-|\+|\x16|\(|\d\x{A0})([\x17-\x1F]*\d++\.?[\x17-\x1F]*)\s++(?=[\x17-\x1F]*[%' . Patterns::CHAR . '\x{b0}-\x{be}\x{2020}-\x{214f}])#mu' => "\$1\xc2\xa0",

            // space between preposition and word
            '#(?<=^|[^0-9' . Patterns::CHAR . '])([\x17-\x1F]*[ksvzouiKSVZOUIA][\x17-\x1F]*)\s++(?=[\x17-\x1F]*[0-9' . Patterns::CHAR . '])#mus' => "\$1\xc2\xa0",
