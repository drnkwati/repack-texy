...
use Repack\Texy;
use Repack\Texy\HandlerInvocation;
use Repack\Texy\LineParser;
use Repack\Texy\Link;
use Repack\Texy\Patterns;
...
final class LinkModule extends Texy\Module
{
    ...
    public function patternReference(LineParser $parser, array $matches)
    {
        ...
        if (!$link) {
            return $texy->invokeAroundHandlers('newReference', $parser, array($name));
        }
        ...
        return $texy->invokeAroundHandlers('linkReference', $parser, array($link, $content));
    }
    ...
    {
        if ($this->texy->obfuscateEmail && preg_match('#^' . self::$EMAIL . '$#u', $link->raw)) {
            // email
